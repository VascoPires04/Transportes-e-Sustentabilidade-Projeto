...
emissions_cars_gasoleo_peak = 6986684.258987817294
print(f"Fator de emissão CO2 (g/km*V) para carros a gasoleo em peak hours = {round(1e6 * emissions_cars_gasoleo_peak/sum(k * s for k, s in zip(kms_gasoleo, stock_gasoleo)), 9)}")


emissions_cars_lpg_peak = 106384.2189615228944
print(f"Fator de emissão CO2 (g/km*V) para carros a lpg em peak hours = {round(1e6 * emissions_cars_lpg_peak/sum(k * s for k, s in zip(kms_lpg, stock_lpg)), 9)}")


############################################################ off

emissions_cars_gasolina_off = 2790302.7164585988245 + 41556.7372806337462
print(f"Fator de emissão CO2 (g/km*V) para carros a gasolina em off hours = {round(1e6 * emissions_cars_gasolina_off/sum(k * s for k, s in zip(kms_gasolina, stock_gasolina)), 9)}")


emissions_cars_gasoleo_off = 6470034.54481024228
print(f"Fator de emissão CO2 (g/km*V) para carros a gasolina em off hours = {round(1e6 * emissions_cars_gasoleo_off/sum(k * s for k, s in zip(kms_gasoleo, stock_gasoleo)), 9)}")


emissions_cars_lpg_off = 100169.5475786643644
print(f"Fator de emissão CO2 (g/km*V) para carros a lpg em off hours = {round(1e6 * emissions_cars_lpg_off/sum(k * s for k, s in zip(kms_lpg, stock_lpg)), 9)}")

## BUSES
emissions_buses = 161992.2273705436231
print(f"Fator de emissão CO2 (g/km*V) para autocarros = {round(1e6 * emissions_buses/sum(k * s for k, s in zip(kms_buses, stock_buses)), 9)}")