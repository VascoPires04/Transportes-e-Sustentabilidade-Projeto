...
emissions_cars_gasolina_peak = 188.4445170643292810 + 0.50419785658208967
print(f"Fator de emissão CH4 (g/km*V) para carros a gasolina em peak hours = {round(1e6 * emissions_cars_gasolina_peak/sum(k * s for k, s in zip(kms_gasolina, stock_gasolina)), 9)}")


emissions_cars_gasoleo_peak = 60.181493566957725394
print(f"Fator de emissão CH4 (g/km*V) para carros a gasoleo em peak hours = {round(1e6 * emissions_cars_gasoleo_peak/sum(k * s for k, s in zip(kms_gasoleo, stock_gasoleo)), 9)}")


emissions_cars_lpg_peak = 12.082339906736917116
print(f"Fator de emissão CH4 (g/km*V) para carros a lpg em peak hours = {round(1e6 * emissions_cars_lpg_peak/sum(k * s for k, s in zip(kms_lpg, stock_lpg)), 9)}")


############################################################ off

emissions_cars_gasolina_off = 188.4445170643292900 + 0.50419785658208968
print(f"Fator de emissão CH4 (g/km*V) para carros a gasolina em off hours = {round(1e6 * emissions_cars_gasolina_off/sum(k * s for k, s in zip(kms_gasolina, stock_gasolina)), 9)}")


emissions_cars_gasoleo_off = 60.211287659147395104
print(f"Fator de emissão CH4 (g/km*V) para carros a gasolina em off hours = {round(1e6 * emissions_cars_gasoleo_off/sum(k * s for k, s in zip(kms_gasoleo, stock_gasoleo)), 9)}")


emissions_cars_lpg_off = 12.082339906736917126
print(f"Fator de emissão CH4 (g/km*V) para carros a lpg em off hours = {round(1e6 * emissions_cars_lpg_off/sum(k * s for k, s in zip(kms_lpg, stock_lpg)), 9)}")



## BUSES
emissions_buses = 37.52444621675
print(f"Fator de emissão CH4 (g/km*V) para autocarros = {round(1e6 * emissions_buses/sum(k * s for k, s in zip(kms_buses, stock_buses)), 9)}")