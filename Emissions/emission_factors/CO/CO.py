...
emissions_cars_gasolina_peak = 25662.87465457691960 + 60.3398344953150012
print(f"Fator de emissão CO (g/km*V) para carros a gasolina em peak hours = {round(1e6 * emissions_cars_gasolina_peak/sum(k * s for k, s in zip(kms_gasolina, stock_gasolina)), 9)}")


emissions_cars_gasoleo_peak = 6061.47996089931707
print(f"Fator de emissão CO (g/km*V) para carros a gasoleo em peak hours = {round(1e6 * emissions_cars_gasoleo_peak/sum(k * s for k, s in zip(kms_gasoleo, stock_gasoleo)), 9)}")


emissions_cars_lpg_peak = 818.6607673034912605
print(f"Fator de emissão CO (g/km*V) para carros a lpg em peak hours = {round(1e6 * emissions_cars_lpg_peak/sum(k * s for k, s in zip(kms_lpg, stock_lpg)), 9)}")


############################################################ off

emissions_cars_gasolina_off = 22491.89866538704917 + 56.5501863378273045
print(f"Fator de emissão CO (g/km*V) para carros a gasolina em off hours = {round(1e6 * emissions_cars_gasolina_off/sum(k * s for k, s in zip(kms_gasolina, stock_gasolina)), 9)}")


emissions_cars_gasoleo_off = 5287.539997413725703
print(f"Fator de emissão CO (g/km*V) para carros a gasolina em off hours = {round(1e6 * emissions_cars_gasoleo_off/sum(k * s for k, s in zip(kms_gasoleo, stock_gasoleo)), 9)}")


emissions_cars_lpg_off = 705.9032112782003412
print(f"Fator de emissão CO (g/km*V) para carros a lpg em off hours = {round(1e6 * emissions_cars_lpg_off/sum(k * s for k, s in zip(kms_lpg, stock_lpg)), 9)}")

## BUSES
emissions_buses = 360.728067551082458
print(f"Fator de emissão CO (g/km*V) para autocarros = {round(1e6 * emissions_buses/sum(k * s for k, s in zip(kms_buses, stock_buses)), 9)}")