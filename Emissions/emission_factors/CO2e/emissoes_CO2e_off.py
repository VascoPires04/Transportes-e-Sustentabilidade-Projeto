...
if profile_column_name not in df.columns:
    # Se a coluna 'Profile' não existir, derive-a a partir da pergunta 22
    q22_column = "Would you use a bike-sharing system at IST and shift to cycling for daily commuting? (Not interested - 0; Definitely yes, even if it is not electric - 1; Definitely yes, if it is  electric - 2; I have to think - 3)"
    
    def classify_profile(resposta):
        if resposta == 1:
            return 1
        elif resposta == 2:
            return 2
            ...
            return 4  # Considera qualquer outra resposta como perfil 4
    
    df['Profile'] = df[q22_column].apply(classify_profile)
else:
...
print(f"\nEmissões Totais Antes: {round(emissoes_totais_antes, 2)} kg CO2e (gases de estufa) (off)")
print(f"Emissões Totais Depois: {round(emissoes_totais_depois, 2)} kg CO2e (gases de estufa) (off)")
print(f"Redução de Emissões: {round(reducao_emissoes, 2)} kg CO2e (gases de estufa) (off)")
