...
emissions_cars_gasolina_peak = 3075.1311948089697767 + 0.324721699839620689
print(f"Fator de emissão VOC (g/km*V) para carros a gasolina em peak hours = {round(1e6 * emissions_cars_gasolina_peak/sum(k * s for k, s in zip(kms_gasolina, stock_gasolina)), 3)}")


emissions_cars_gasoleo_peak = 1047.1242742221448145
print(f"Fator de emissão VOC (g/km*V) para carros a gasoleo em peak hours = {round(1e6 * emissions_cars_gasoleo_peak/sum(k * s for k, s in zip(kms_gasoleo, stock_gasoleo)), 3)}")


emissions_cars_lpg_peak = 132.590901535487169041
print(f"Fator de emissão VOC (g/km*V) para carros a lpg em peak hours = {round(1e6 * emissions_cars_lpg_peak/sum(k * s for k, s in zip(kms_lpg, stock_lpg)), 3)}")


############################################################ off

emissions_cars_gasolina_off = 2702.7684867794817947 + 0.289538261294710883
print(f"Fator de emissão VOC (g/km*V) para carros a gasolina em off hours = {round(1e6 * emissions_cars_gasolina_off/sum(k * s for k, s in zip(kms_gasolina, stock_gasolina)), 3)}")


emissions_cars_gasoleo_off = 905.8924425266751029
print(f"Fator de emissão VOC (g/km*V) para carros a gasolina em off hours = {round(1e6 * emissions_cars_gasoleo_off/sum(k * s for k, s in zip(kms_gasoleo, stock_gasoleo)), 3)}")


emissions_cars_lpg_off = 114.078761828982725630
print(f"Fator de emissão VOC (g/km*V) para carros a lpg em off hours = {round(1e6 * emissions_cars_lpg_off/sum(k * s for k, s in zip(kms_lpg, stock_lpg)), 3)}")

## BUSES
emissions_buses = 106.4561320063410158
print(f"Fator de emissão VOC (g/km*V) para autocarros = {round(1e6 * emissions_buses/sum(k * s for k, s in zip(kms_buses, stock_buses)), 3)}")