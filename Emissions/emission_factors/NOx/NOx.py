...
emissions_cars_gasolina_peak = 3605.540958847604980 + 2.689957120737306543
print(f"Fator de emissão NOx (g/km*V) para carros a gasolina em peak hours = {round(1e6 * emissions_cars_gasolina_peak/sum(k * s for k, s in zip(kms_gasolina, stock_gasolina)), 9)}")


emissions_cars_gasoleo_peak = 23166.115913274460772
print(f"Fator de emissão NOx (g/km*V) para carros a gasoleo em peak hours = {round(1e6 * emissions_cars_gasoleo_peak/sum(k * s for k, s in zip(kms_gasoleo, stock_gasoleo)), 9)}")


emissions_cars_lpg_peak = 166.00227387533383626
print(f"Fator de emissão NOx (g/km*V) para carros a lpg em peak hours = {round(1e6 * emissions_cars_lpg_peak/sum(k * s for k, s in zip(kms_lpg, stock_lpg)), 9)}")


############################################################ off

emissions_cars_gasolina_off = 3490.783041692420047 + 2.66440008285441299
print(f"Fator de emissão NOx (g/km*V) para carros a gasolina em off hours = {round(1e6 * emissions_cars_gasolina_off/sum(k * s for k, s in zip(kms_gasolina, stock_gasolina)), 9)}")


emissions_cars_gasoleo_off = 21376.024978780944586
print(f"Fator de emissão NOx (g/km*V) para carros a gasolina em off hours = {round(1e6 * emissions_cars_gasoleo_off/sum(k * s for k, s in zip(kms_gasoleo, stock_gasoleo)), 9)}")


emissions_cars_lpg_off = 164.33539817027067714
print(f"Fator de emissão NOx (g/km*V) para carros a lpg em iff hours = {round(1e6 * emissions_cars_lpg_off/sum(k * s for k, s in zip(kms_lpg, stock_lpg)), 9)}")

## BUSES
emissions_buses = 1386.824477017213423
print(f"Fator de emissão NOx (g/km*V) para autocarros = {round(1e6 * emissions_buses/sum(k * s for k, s in zip(kms_buses, stock_buses)), 9)}")